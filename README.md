...
│       ├── knowledge/          # 知识库 v1：规则/出处 JSON + 一致性守卫测试
│       ├── store/              # IndexedDB 本地存储
│       └── ui/                 # 屏幕：今天 / 旅程（路线地图）/ 记录 / 训练 / 收据 / 筛查
├── V1_PRODUCT_CONTRACT.md      # 产品合同（对象模型、安全红线、规则清单）
├── DESIGN.md                   # 视觉系统：国家公园海报语言 + 证据几何标记
├── ROADMAP_v0_2.md             # 当前版本范围与里程碑
├── 04-task-playbooks/          # 任务执行手册（用户测试执行包）
├── 05-decisions/               # 架构决策记录（ADR）
├── 06-plans/                   # 实施计划归档（会话 plan 定稿后入库）
├── 07-partner/                 # 伙伴产出的 PRD 与交互原型（FitJourney）
├── *-v0-*.html                 # 设计原型（海报样稿、流程原型，可直接浏览器打开）
└── *.md                        # 产品框架 / 市场研究 / 验证计划
```

> 编号说明：`01-03` 未使用——产品框架、市场研究、视觉设计三类文档以根目录散文档形式存在（`PRODUCT_FRAMEWORK_V0_1.md`、`RESEARCH_*.md`、`EXPERIENCE_VISUAL_DIRECTION_V0_1.md` 等），编号目录从 `04` 起。
